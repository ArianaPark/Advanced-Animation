class Snake{
  ...
  update(){
    ...
    var distance  = new Vector(this.head.x - this.array[0].x,this.head.y - this.array[0].y);
    ...
    if(distance.x<0){ //if x is decreasing, going left
      this.array[0].x = this.head.x + (Math.cos(theta)*20);
    } else{ //going right
      this.array[0].x = this.head.x - (Math.cos(theta)*20);
    }
    if(distance.y<0){ //going up
      this.array[0].y = this.head.y + (Math.sin(theta)*20);
    } else{//going down
      this.array[0].y = this.head.y - (Math.sin(theta)*20);
    }
    this.draw(0);
